        ...
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                ...
                <div class="bg-light p-3 rounded mb-4">
                    ...
                    @else
                    <div class="alert alert-warning mb-0 py-2">Số dư không đủ. Vui lòng nạp thêm <strong>{{
                            number_format($totalDue - $balance, 0, ',', '.') }} ₫</strong> để thanh toán.</div>
                    @endif
                    @endif
                </div>
                ...
                <form method="POST" action="{{ route('orders.pay', $order->id) }}" class="mb-3">
                    @csrf
                    <button class="btn btn-success w-100 py-2 fw-bold">Thanh toán ngay ({{ number_format($totalDue, 0,
                        ',', '.') }} ₫)</button>
                </form>
                ...
                <div id="qrWrap" class="text-center mt-4" style="display: none;">
                    <img id="qrImage" src="" alt="QR Nap Tien" width="250" height="250"
                ...
                <form id="addMoneyForm" method="POST"
                    action="{{ route('user.add-money.post') }}{{ $order ? '?order_id=' . $order->id : '' }}"
                    class="d-none">
                    @csrf
...
<script>
    (function () {
        ...
        var qrTriggerUrl = @json($qrTriggerUrl);
